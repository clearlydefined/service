...
router.get('/:type/:provider/:namespace/:name/:revision/pr/:pr', asyncMiddleware(async (request, response, next) => {
...
router.patch('/:type/:provider/:namespace/:name/:revision', asyncMiddleware(async (request, response, next) => {
  const packageCoordinates = utils.toPackageCoordinates(request);
