...
// This method will build URLs for any protocol with URLs that can be predicted from the identity of the component
function buildUrls(spec) {
  const registry = buildRegistryUrl(spec)
  const version = buildVersionUrl(spec)
  const download = buildDownloadUrl(spec)
  return { registry, version, download }
}
...
function buildRegistryUrl(spec) {
  switch (spec.provider) {
    ...
    case 'cocoapods':
      return `https://cocoapods.org/pods/${spec.name}`
    default:
      return null
  }
}
