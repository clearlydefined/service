...
function buildHomepageUrl(spec) {
  switch (spec.provider) {
    case 'github':
...
function buildComponentUrl(spec) {
  switch (spec.provider) {
    case 'github':
      return `${buildHomepageUrl(spec)}/tree/${spec.revision}`
    case 'npmjs':
      return `${buildHomepageUrl(spec)}/v/${spec.revision}`
    case 'nuget':
      return `${buildHomepageUrl(spec)}/${spec.revision}`
    case 'cratesio':
      return `${buildHomepageUrl(spec)}/${spec.revision}`
    case 'mavencentral':
      return `${buildHomepageUrl(spec)}/${spec.revision}`
    case 'pypi':
      return `${buildHomepageUrl(spec)}/${spec.revision}`
    case 'rubygems':
      return `${buildHomepageUrl(spec)}/versions/${spec.revision}`
    default:
      return null
  }
}
...
async function buildDownloadUrl(spec) {
  switch (spec.provider) {
    case 'github':
      return `${buildHomepageUrl(spec)}/archive/${spec.revision}.zip`
    case 'npmjs':
    ...
    default:
      return `https://pypi.org/pypi/${spec.name}/json`
  }
}
...
async function pypiUrl(spec) {
  ...
  return registryData[spec.revision] ? registryData[spec.revision].download.url : ''
}
