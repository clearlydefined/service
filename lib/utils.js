...
/**
 * Create ResultCoordinates from an Express request's params.
 * @param {express.Request} request - HttpRequest.
 * @returns {Promise<ResultCoordinates>} resolves to a ResultCoordinates instance
 */
...
/**
 * Create EntityCoordinates from an Express request's params and normalize them.
 * @param {express.Request} request - HttpRequest.
 * @returns {Promise<EntityCoordinates>} resolves to a normalized EntityCoordinates instance
 */
...
/**
 * Normalize a coordinates spec into an EntityCoordinates instance.
 * @param {EntityCoordinates|import('express-serve-static-core').ParamsDictionary} spec - coordinates object or EntityCoordinates
 * @returns {Promise<EntityCoordinates>} resolves to a normalized EntityCoordinates
 */
...
/**
 * Create EntityCoordinates from a plain args object (CLI or similar).
 * @param {any} args - arguments containing `type`, `provider`, `namespace`, `name`, `revision`.
...
/**
 * Re-encode forward slashes in a namespace as `%2f` for routing compatibility.
...
/**
 * Build a namespace/name/revision string including optional extra segments.
 * @param {express.Request} request - HttpRequest.
 * @returns {string}
 */
...
/**
 * Return the latest semver-valid non-prerelease version from an array.
 * If a single value is provided that is not an array it will be returned as-is.
...
/**
 * Normalize and deduplicate attribution entries.
...
/**
 * Parse a date/time string into a Luxon DateTime in UTC if possible.
...
/**
 * Extract an ISO date (YYYY-MM-DD) from various date string formats.
 * @param {string} dateAndTime
 * @returns {string|null}
 */
...
/**
 * Compare two ISO date strings returning positive, negative or zero.
 * @param {string|null} dateA - ISO date string
 * @param {string|null} dateB - ISO date string
 * @returns {number}
 */
function compareDates(dateA, dateB) {
  if (!dateA || !dateB) return dateA ? 1 : dateB ? -1 : 0
  return DateTime.fromISO(dateA).toJSDate().getTime() - DateTime.fromISO(dateB).toJSDate().getTime()
}

/**
 * Set a deep value on `target` at `path` only if `value` is present.
 * @param {any} target
 * @param {string} path
 * @param {any} value
...
/**
 * Convert iterable values to a sorted array, or null if empty.
 * @template T
...
/**
 * Add elements from an array into a Set, optionally projecting values.
 * @template T
 * @param {T[]} array
 * @param {Set<T>} set
 * @param {(item:T)=>T|null} [valueExtractor]
 * @returns {Set<T>}
 */
...
/**
 * Extract a SPDX license identifier from a license URL using overrides.
...
// Be careful to handle various arrays correctly by 
// finding matching entries and merging them as appropriate.
/**
 * Merge two objects, with the option to override existing values.
 * @param {any} base 
 * @param {any} proposed 
...
/**
 * Merge two file lists, with the option to override existing values.
 * @param {any[]} base 
 * @param {any[]} proposed 
 * @param {boolean} override 
 * @returns {any[]} The combined file list
 */
 ...
 * @param {any} proposed 
 * @param {boolean} override 
 * @returns {any} The combined file entry
 */
function _mergeFile(base, proposed, override) {
  ...
  const overrideStrategy = override 
    ? (/** @type {any} */ proposed) => proposed 
    : (/** @type {any} */ proposed, /** @type {(arg0: any) => any} */ mergeStrategy) => mergeStrategy(proposed)
  setIfValue(
    result,
    'license',
...
/**
 * Merge two objects except for specified paths.
 * @param {any} base 
 * @param {any} proposed 
 * @param {string[]} paths 
 * @returns {any}
...
/**
 * Build a source URL for the given coordinates.
 * @param {EntityCoordinates} spec
 * @returns {string|null}
 */
...
/**
 * Decode URL-encoded slashes in a namespace string.
 * @param {string} namespace 
 * @returns {string}
 */
 ...
 * @param {any[]?} [packages] - optional, to look at package directories
 * @returns {boolean}
 */
...
function debsrcLicenseLocations(packages) {
  ...
  return licenseLocations.concat(
    otherPackages.map((/** @type {{ version: any; name: any; }} */ otherPackage) =>
...
/**
 * @param {Iterable<any>} expressions
 * @returns { string | null }
 */
...
/**
 * 
 * @param {string} rawLicenseExpression 
 * @param {*} logger 
 * @param {SPDX.LicenseRefLookup} licenseRefLookup 
 * @returns { string | null}
 */
function normalizeLicenseExpression(
  ...
  const licenseVisitor = (/** @type {string} */ licenseExpression) =>
...
/**
 * Normalize a version string to semver, returning null if not valid.
 ...
 * @param {string} urn 
 * @returns { {scheme: string, type: string, provider: string, namespace: string, name: string, revToken: string, revision: string, toolToken: string, tool: string, toolRevision: string } | null }
 */
