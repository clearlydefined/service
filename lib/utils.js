...
const { set, unset, union, sortBy, trim, uniqBy, map, get, forEach } = require('lodash')
...
async function buildDownloadUrl(spec, raw) {
  switch (spec.provider) {
    case 'github':
...
async function pypiUrl(spec, raw) {
  if (!raw) return null
  let registryDataUrl
  await forEach(raw, tool => {
    forEach(tool, content => {
      if (!get(content, 'registryData.releases')) return
      forEach(content.registryData.releases[spec.revision], revision => {
        if (revision.filename.includes('tar.gz')) registryDataUrl = revision.url
      })
    })
  })
  return registryDataUrl || null
}
