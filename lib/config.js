...
module.exports = {
  ...
  component: {
    store: {
      provider: config.get('COMPONENT_STORE_PROVIDER') || 'file',
      azblob: {
        connectionString: config.get('COMPONENT_AZBLOB_CONNECTION_STRING'),
        containerName: config.get('COMPONENT_AZBLOB_CONTAINER_NAME') || `component-${process.env.NODE_ENV}`
      },
      file: {
        location: config.get('FILE_STORE_LOCATION') || process.platform === 'win32' ? 'c:/temp/cd-component' : '/tmp/cd-component'
      }
    }
  },
  auth: {
