...
class RedisBackedRateLimiter extends RateLimiter {
  ...
  async _initializeClient() {
    ...
    const client = RedisBackedRateLimiter.buildRedisClient(this.options.redis)
    client.on('error', error => {
      this.logger.error(`Redis client error: ${error}`)
    })
    await client.connect().then(() => this.logger.info('Done connecting to redis: %s', this.options.redis.service))
    return client
  }
