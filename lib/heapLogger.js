  ...
  if (shouldLogHeapstats) {
    ...
    const logHeapSpaceStats = () => {
      ...
      for (const space of heapSpaceStats) {
        const heapStatsMessage = `[${currentTimestamp}] Heap Space Statistics: Space Name: '${space.space_name}', Space Size: '${addCommas(space.space_size)}' bytes, Space Used Size: '${addCommas(space.space_used_size)}' bytes, Space Available Size: '${addCommas(space.space_available_size)}' bytes, Physical Space Size: '${addCommas(space.physical_space_size)}' bytes\n--------------------------`
